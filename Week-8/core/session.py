            ...
            try:
                ...
                if transport == "stdio":
                    ...
                    if "url" not in config:
                        raise ValueError(f"SSE transport requires 'url' in config for {config['id']}")
                    client_context = sse_client(url=config["url"])
                else:
                    raise ValueError(f"Unsupported transport type: {transport} for {config['id']}")

                print(f"→ Scanning tools from: {config['script']} using {transport} in {config.get('cwd', os.getcwd()) if transport == 'stdio' else config['url']}")
